...
- IMAGE STYLE — think scroll-stopping THUMBNAIL, not a clean corporate instructional diagram. Use
  ONE of these two directions:
  (a) A bold, high-contrast HEADLINE-style title card with a real/photographic-feeling hero visual
      behind or around it (a person, a hand holding something, a dramatic real-world scene) — like a
      YouTube thumbnail or Facebook ad, the kind of image that makes someone stop mid-scroll.
  (b) A fun illustrated character/mascot personifying the story (e.g. rival companies or AI models
      drawn as characters reacting to each other) when the topic suits humor or rivalry — this style
      has driven the strongest engagement for this account historically.
  AVOID multi-panel numbered "how it works" diagrams, small icon grids, or textbook-style step
  breakdowns — those read as documentation, not as something worth stopping for. The headline text in
  the image must be BIG, bold, and high-contrast against the background — not a small label.
...
- IMAGE: set needs_image true when a visual would help (default true for tool/multi-item content).
  Think scroll-stopping THUMBNAIL, not a clean corporate diagram: either (a) a bold headline-style
  title card with a real/photographic-feeling hero visual (a person, a hand, a dramatic scene) — like
  a YouTube thumbnail; or (b) a fun illustrated character/mascot personifying the tools/rivalry when
  humor fits. Avoid multi-panel numbered diagrams or small icon grids — those read as documentation,
  not something worth stopping for. Big bold high-contrast title text, not a small label. Vary the
  color palette and hero visual each time so the account doesn't look like it's using one template.
