...
def fetch_hn_weekly_top(min_points=60, limit=25):
...
def fetch_producthunt_top(limit=25):
...
Below are up to {n} AI tools/stories that are trending THIS WEEK based on live rankings (Hacker News
...
- Only include items that are genuinely interesting enough to write a real post about. Drop anything
  ...
  are all genuinely good, write 15. Quality over quantity, but don't artificially cap a strong week.
- Come up with a bold, catchy TITLE for this week's roundup (like a mini headline, not just
  "Weekly AI Roundup") — this gets its own field, don't put it inside the posts. The title should
  match the actual hook used in post 1, not feel like a separate tagline.
- Post 1: a hook in the first 8-10 words that frames this as this week's trending AI finds. NEVER open
  with throat-clearing like "Lately I've been...", "I've been digging into...", "So I found..." — lead
  with the number/fact/claim itself.
- Then ONE post per item you kept: say what it is and give a one-line reaction/why-it-matters, based
  ONLY on the title/context given. Do NOT invent specific features, pricing, or capabilities not
  implied by the title — if the title is vague, keep that post short and honest instead of making
  things up. Don't just reword the source title — react to it.
- Final post: a question or hot take to the audience. No filler ("stay tuned", etc). Include 1-3
  broad, high-traffic hashtags in this final post (#AI, #AITools, #MachineLearning) — avoid redundant
  pairs like "#AIML #ML" together. Hashtags render as clickable blue links on X and help visibility
  even though they don't drive the algorithm much.
...
- IMAGE DECISION: Set needs_image true whenever a visual would plausibly boost engagement — most
  tool posts, anything visually striking, benefit from an image. Only set false if genuinely nothing
  visual applies. When in doubt, default to true.
- IMAGE STYLE — think a well-designed PowerPoint/slide-CARD with a bold hook headline as the visual
  anchor, not an instructional diagram. A hero photo or character is OPTIONAL, not required — a pure
  text+icon card (bold headline + a few supporting icons naming the tools covered, no photo at all) is
  a perfectly good option, as is a clean grid/list layout. AVOID multi-panel numbered "how it works"
  diagrams — those read as tutorial slides, not a hook-driven roundup image. Headline text should be
  big, bold, high-contrast — not a small label buried among icons.
...
{{
  "title": "bold catchy title for this week's roundup",
  "posts": ["post 1 (intro/hook)", "post 2 (item 1)", "... one per KEPT item ...", "final post w/ hashtags"],
...
def send_roundup_to_telegram(items, roundup_data):
    ...
    title = roundup_data.get("title", "Weekly AI Roundup")
    header = f"📅 <b>{_escape_html(title)}</b> ({len(posts)} posts)"
    _tg_send(header)
    ...
    if _is_true(roundup_data.get("needs_image")) and roundup_data.get("image_prompt"):
        img_msg = f"🎨 <b>Image Prompt (use with ChatGPT/Gemini/Nano Banana):</b>\n<pre>{_escape_html(roundup_data['image_prompt'])}</pre>"
        _tg_send(img_msg)

    # Only list items actually covered in the posts, not every candidate we fetched —
    # keeps the link list matching what the thread actually discusses.
    kept, _ = filter_kept_items(items, roundup_data.get("dropped_titles"))
...
def main():
    ...
    # Only mark items actually covered as posted — dropped ones stay available
    # to be reconsidered another week instead of being burned from the pool.
    kept, dropped = filter_kept_items(items, roundup_data.get("dropped_titles"))
    for it in kept:
        state.append({"hash": it["_hash"], "link": it["link"], "title": it["title"], "ts": int(time.time())})
    save_state(state)
    if dropped:
        print(f"[info] {len(dropped)} candidate(s) dropped this week, left available for later")
    print(f"[ok] posted weekly roundup with {len(kept)} kept item(s)")
