...
- IMAGE STYLE: this account posts news/tutorials/tool reviews, not fiction — so image prompts should
  ...
  Think: dark tech-dashboard aesthetic, clean sans-serif on-image text labels, small icons, callout
  boxes with short data points or feature names, a clear header title — like a product screenshot or
  data-viz slide, not a dramatic movie-poster scene. Explicitly include what text/labels should appear
  in the image (a short title, 2-4 short labels/callouts relevant to THIS specific story) since
  modern AI image tools (Gemini/Nano Banana, ChatGPT/GPT Image, etc.) render text well now —
  a wordless "artistic" image is a missed opportunity here.
...
def main():
    ...
    # try the target category first, then fall back to everything else
